...
# -------------------------------------------------------
run "$BWA aln -l22 -k0 -t25 $HG_REF $STRAND_1 > $ALN_HU_1"

run "$BWA aln -l22 -k0 -t25 $HG_REF $STRAND_2 > $ALN_HU_2"

run "$BWA sampe -r'$RG' $HG_REF $ALN_HU_1 $ALN_HU_2 $STRAND_1 $STRAND_2 | $SAMTOOLS view -@25 -bS -F 0x4 - > $HH_BAMFILE"
...
run "$BWA aln -l22 -k0 -t25 $MG_REF $HH_FASTQ_1 > $ALN_MSE_1"

run "$BWA aln -l22 -k0 -t25 $MG_REF $HH_FASTQ_2 > $ALN_MSE_2"

run "$BWA sampe -r'$RG' $MG_REF $ALN_MSE_1 $ALN_MSE_2 $HH_FASTQ_1 $HH_FASTQ_2 | $SAMTOOLS view -@25 -bS -f 0x4 - > $MSE_BAMFILE"
...
run "$BWA aln -l22 -k0 -t25 $HG_REF $HO_FASTQ_1 > $ALN_HO_1"

run "$BWA aln -l22 -k0 -t25 $HG_REF $HO_FASTQ_2 > $ALN_HO_2"

run "$BWA sampe -r'$RG' $HG_REF $ALN_HO_1 $ALN_HO_2 $HO_FASTQ_1 $HO_FASTQ_2 | $SAMTOOLS view -@25 -bS -F 0x4 - > $HO_BAMFILE"
...
run "java -Xmx50g -jar $PICARD MarkDuplicates I=${HH_SORTED}.bam O=$HH_BAMFILE METRICS_FILE=$HH_METRICS_FILE && rm ${HH_SORTED}.bam $HH_METRICS_FILE"

run "$SAMTOOLS index $HH_BAMFILE"

run "$SAMTOOLS sort -@ 25 $HO_BAMFILE $HO_SORTED"

run "java -Xmx50g -jar $PICARD MarkDuplicates I=${HO_SORTED}.bam O=$HO_BAMFILE METRICS_FILE=$HO_METRICS_FILE && rm ${HO_SORTED}.bam $HO_METRICS_FILE"
...
# ------- HO ---------
run "java -Xmx100g -jar $GATK -T UnifiedGenotyper -nt 15 -glm BOTH -R $HG_REF -dcov 5000 -I $HO_BAMFILE --output_mode EMIT_VARIANTS_ONLY -l OFF -stand_call_conf 1 -L $TARGET_REGIONS > $HO_ALL_VARIANTS"
...
run "perl $VEP -i $HO_PASS_EXTRACTED -o $HO_VEP_ANNO --cache --vcf --verbose --everything --fork 20 --total_length --maf_1kg --check_existing --allele_number --check_svs --port 3337 --buffer_size 50000000 --dir /12TBLVM/Data/VEP79cache --force_overwrite && rm $HO_PASS_EXTRACTED"
