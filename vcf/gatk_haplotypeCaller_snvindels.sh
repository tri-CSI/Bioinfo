...
# Pipeline for calling variants using GATK HaplotypeCaller and Ensembl VEP annotation with EAS_MAF and SAS_MAF filter
# version 1.0.1
...
TARGET_REG_PREFIX="/12TBLVM/Data/MinhTri/REFERENCES/NexteraRapidCapture_Exome_TargetedRegions_v1.2Used_"
LOG_FILE="HaplotypeCaller.$(date +%Y%m%d%H%M%S).log"
HU_BAMFILE=$1
...
echo "Description : Run GATK HaplotypeCaller, VEP and ASN_MAF filtering" >> $LOG_FILE
...
# -------------------------------------------------------
java  -Djava.io.tmpdir="/tmp" -Xmx${ram}g -jar $GATK -T HaplotypeCaller -R $HG_REF -dcov 5000 -I $HU_BAMFILE -o $ALL_VARIANTS --output_mode EMIT_VARIANTS_ONLY -l OFF -stand_call_conf 1 -L ${TARGET_REG_PREFIX}00.bed &

for number in $(seq -f "%02g" 1 29)
do
	target_int="${TARGET_REG_PREFIX}${number}.bed"
	scatter_file="haplotype_${number}.vcf"

	java  -Djava.io.tmpdir="/tmp" -Xmx${ram}g -jar $GATK -T HaplotypeCaller -R $HG_REF -dcov 5000 -I $HU_BAMFILE -o $scatter_file --output_mode EMIT_VARIANTS_ONLY -l OFF -stand_call_conf 1 -L $target_int &
done
...
do
	...
	grep "^chr*" $scatter_file >> $ALL_VARIANTS && rm $scatter_file ${scatter_file}.idx
done
