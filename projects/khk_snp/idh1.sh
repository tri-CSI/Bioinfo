...
FINAL="${BASE_NAME}.IDH1.txt"

start_time=$(date +%s)
last_time=$(date +%s)


# Extract IDH1 chr2:209,100,953-209,119,806
command="awk '{if ((\$1~/#/) || (\$1==\"chr2\" && \$2>=209100953 && \$2 <=209119806)) print}' $ORIGINAL_VCF > $EXTRACTED"
...
run "$command" $FINAL
echo Total IDH1 variants of $BASE_NAME: `cat $FINAL | grep -v "#" | wc -l`
