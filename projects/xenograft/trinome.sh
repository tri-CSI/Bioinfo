...
if [ $2 -eq 0 ]; then
	echo "Process succeeds, file(s) created: $3" >> ${LOG_FILE}
else
...
echo " + Process runtime: $hours hrs $minutes mins $seconds secs" >> ${LOG_FILE}

time_elapsed=$[ `date +%s` - $start_time ]
hours=$[ $time_elapsed / 3600 ]
minutes=$[ $[$time_elapsed % 3600] / 60]
seconds=$[ $[$time_elapsed % 3600] % 60]
echo " + Total time elapsed: $hours hrs $minutes mins $seconds secs" >> ${LOG_FILE}
}
